...
func New(db *sql.DB, logger *log.Logger) domain.TokenRepository {
	return &repository{
		db:     db,
...
func (r *repository) CreateToken(ctx context.Context, in entity.TokenEntity) (int64, error) {
	create := sqlbuilder.NewStruct(in)
	sb := create.InsertInto(domain.TokenTable, &in)
	query, args := sb.Build()
...
func (r *repository) ReadToken(ctx context.Context, request entity.CollectionRequest, result interface{}) error {
	...
	return utils.NewNotFoundError("token", "filters", request.Filters)
}

func (r *repository) UpdateToken(ctx context.Context, request entity.CollectionRequest) error {
